...
function TakeTurnState:attack(attacker, defender, attackerSprite, defenderSprite, attackerkBar, defenderBar, onEnd)
	...
	Timer.after(0.5, function()
		...
		:finish(function()
			...
			gSounds['hit']:play()
			Timer.every(0.1, function()
				defenderSprite.opacity = defenderSprite.opacity == 0.3 and 1 or 0.3
			end)
			:limit(6)
			:finish(function()
...
function TakeTurnState:victory()
	...
	:finish(function()
		...
		function()
			local exp = (self.opponentPokemon.HPIV + self.opponentPokemon.attackIV + self.opponentPokemon.defenseIV + self.opponentPokemon.speedIV) * self.opponentPokemon.level
